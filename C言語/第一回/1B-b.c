...
// 絶対値に関して昇順に並んでいる場合に、
// 線形リストの新しいノードを挿入すべき位置を返す関数
int seach_position(data_type x, node_type *p)
{
  int i;
...
// 絶対値に関して昇順に並んでいる場合に、
// 線形リストに新しいノードを挿入する関数
int insert_ascending_order(data_type x, node_type **pp)
{
  node_type *temp;
  int insert_position = seach_position(x, *pp);
...
int main(int argc, char *argv[])
{
  ...
  {
    sscanf(buf, "%lf", &temp);
    insert_ascending_order(temp, &head);
  }
